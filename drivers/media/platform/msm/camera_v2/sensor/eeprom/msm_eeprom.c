				       ...
				       struct msm_eeprom_cfg_data *cdata)
{
	...
	if (!rc) {
/*When read happens for deamon kill , data needs to be preserved*/
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (emap[j].page.valid_size) {
			...
			rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
				...
				pr_err("%s: %d  client->addr_type  = %d \n", __func__, __LINE__ , e_ctrl->i2c_client.addr_type);
			if (rc < 0) {
		...
		if (emap[j].mem.valid_size) {
			...
			pr_err("%s: %d  emap[j].mem.addr  = %d emap[j].mem.valid_size = %d   memptr = %p\n", __func__, __LINE__ ,emap[j].mem.addr, emap[j].mem.valid_size, memptr);
			if (rc < 0) {
			 ...
			 const struct i2c_device_id *id)
{
	...
	power_info->dev = &client->dev;
        ...
        if (e_ctrl->cal_data.map) {
                ...
                e_ctrl->is_supported |= msm_eeprom_match_crc(
                                                &e_ctrl->cal_data);
		  pr_err("e_ctrl->is_supported rc %d\n", e_ctrl->is_supported);
        }
